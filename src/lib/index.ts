 ...
 */
const intervals = new WeakMap<Element, Timer>();
/**
 ...
 */
const debounces = new WeakMap<Element, Timer>();
/**
 ...
 * @returns
 */
function getTransitionSizes(el: Element, oldCoords: Coordinates, newCoords: Coordinates) {
...
function getOptions(el: Element): AutoAnimateOptions {
	const defaultOptions = { duration: 250, easing: 'ease-in-out' };
	if (TGT in el) {
		const option = options.get((el as Element & { __aa_tgt: Element })[TGT]);
		if (!option) return defaultOptions;
		return option;
	}
...
function remove(el: Element) {
	...
	const tuple = siblings.get(el);
	if (!tuple) return;
	const [prev, next] = tuple;
	...
	if (finalX !== scrollX || finalY !== scrollY) {
		adjustScroll(el, finalX, finalY, options);
	}
 ...
 * @returns
 */
function adjustScroll(el: Element, finalX: number, finalY: number, options: AutoAnimateOptions) {
	...
	function smoothScroll() {
		requestAnimationFrame(() => {
			...
			if (startScroll + options.duration > performance.now()) {
				window.scrollTo({
					...(deltaX ? { left: window.scrollX - deltaX } : {}),
					...(deltaY ? { top: window.scrollY - deltaY } : {})
				});
...
export function flip(
	...
	if (mutations && resize) {
		...
		const isDisabledDueToReduceMotion =
			mediaQuery.matches && !config.disrespectUserMotionPreference;
